...
export async function getNotifications(uid: string, maxResults = 50): Promise<Notification[]> {
  const notifs = await getDocs(
    query(COLLECTION(uid), orderBy("createdAt", "desc"), limit(maxResults))
  );
...
export function subscribeToNotifications(
  ...
  return onSnapshot(query(COLLECTION(uid), orderBy("createdAt", "desc"), limit(50)), (snap) => {
    onChange(
      snap.docs.map((d) => ({
        id: d.id,
        ...(d.data() as Omit<Notification, "id">),
