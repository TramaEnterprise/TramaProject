...
import {
  ...
  saveAuthorToDB,
  updateAuthorBioToDB,
} from "@/services/firebase/firebaseAuthors";
    ...
    const fetchAuthorData = async () => {
      ...
      if (authorKey) {
        try {
          ...
          if (dbAuthorData) {
            ...
            saveAuthorToDB(
              authorKey,
              { key: authorKey, name: authorName, bio, photoUrl },
              lang
            ).catch(() => {});
          }
        } catch {
      ...
      if (authorKey) {
        try {
          ...
          /* Si falla Firestore o no hay suficientes libros, se llama a API */
        }
      }
      ...
      if (books.length < 2) {
        try {
          ...
          /* si falla OpenLibrary, books queda vacío */
        }
      }
